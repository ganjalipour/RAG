...
from rag_flow.embedding_managernew import EmbeddingManager
...
class ContextAwareAgent(Agent):
    ...
    def __init__(self, bot_id: str, language: str, bot_config: dict, call_start_time: float = 0) -> None:
        ...
        self.real_assistant_called = False 
        self.__init_embed()
    ...
    def __init_embed(self):
        ...
        try:
            ...
            config.qdrant_url = "https://030204a0-d677-4608-abae-9886fe552ce4.eu-central-1-0.aws.cloud.qdrant.io:6333"
            config.collection_name = "rag_embeddings"
            config.qdrant_api_key = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJhY2Nlc3MiOiJtIn0.jxJCFk4DogmElkTus0kBi5vGszFRX1Y0APwJoBzKvGo"
    ...
    async def _search_knowledge_base_direct(self, query: str) -> str:
        ...
        try:
            results = self.embedding_manager.get_context_for_query_qdrant(
                query=query,
...
class VoiceAgent:
    ...
    def setup_speech(self, language: str, lang_code: str):
        ...
        if language in DEEPGRAM_CARTESIA_LANGUAGES:
            # Deepgram + Cartesia برای زبان‌های پشتیبانی‌شده
            tts = cartesia.TTS(
                api_key=CONFIG["cartesia_api_key"],
                voice=CONFIG["cartesia_voice_id"] if lang_code == "tr" else TTSDefaultVoiceId,
            ...
            stt = openai.STT(
                api_key=CONFIG["openai_api_key"],
                model="gpt-4o-transcribe", # "gpt-4o-mini-transcribe",  # مدل دقیق‌تر و سبک‌تر از gpt-4o-transcribe
                language="fa",
                prompt="default language is persian"
            )
            # stt = LocalWhisperSTT(model_size="small", language="fa")
        return stt, tts
    ...
    async def initialize(self, ctx: JobContext) -> None:
        ...
        #     stt=stt,
        #     llm=openai.LLM(model=CONFIG["openai_model_name"]),
        #     tts=tts,
        #     vad=silero.VAD.load(
        ...
        self.session = AgentSession(
            llm=openai.realtime.RealtimeModel(model='gpt-4o-realtime-preview-2025-06-03', voice='sage', 
                                            input_audio_transcription=realtime.AudioTranscription(
                                                ...
                                                language="fa", 
                                                prompt=f"default langauge for transcribe is: \"{language}\"",  # ✅ پرامپت فارسی
                                            ),
                                              ),
            vad=silero.VAD.load(
                activation_threshold=CONFIG["activation_threshold"],
