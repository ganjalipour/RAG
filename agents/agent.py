...
class ContextAwareAgent(Agent):
    ...
    async def on_user_turn_completed(
        ...
        if new_message == "#" : 
            ...
            #raise StopResponse()
            turn_ctx.shutdown(reason="User pressed # to transfer to real assistant")
        
        print("newwwwwwww1111111")
        print(new_message)
        if self.real_assistant_called:
            print("newwwwwwww22222222222222")
            LOG.info("Agent muted, skipping reply in pipeline.")
    ...
    async def call_real_assistant(self, phone_number: str = "+905325747455", user_message: str = ""):
        ...
        trigger_words = ["asistan","asistana","asistana bağla", "gerçek asistan", "canlı destek", "operatör", "insan","insana"]
        print(user_message)
        ...
        try:
            ...
            LOG.info(f"Gerçek asistana bağlanıyor... room={ctx.room.name}")
            ...
            LOG.error(f"SIP çağrısı başarısız: {e}")
