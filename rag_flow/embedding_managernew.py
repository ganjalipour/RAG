...
# from rag_flow.embedding_store import EmbeddingStore
...
class EmbeddingManager:
    ...
    #         print(f"🔹 بارگذاری مدل embedding: {model_name}")
    ...
    def search_similar_chunks_qdrant(
        ...
        **search_kwargs,
        ...
        try:
            strategy = SearchStrategyFactory.create_strategy(SearchMode.VECTOR)
            print("66666")
            where_filter = self._build_where_filter(active_only, source_filter)
            ...
            LOG.error(f"Error in search khata ({search_mode.value}): {e}")
            return []
    ...
    def search_similar_chunks(
        ...
        **search_kwargs,
        ...
        try:
            ...
            # ✅ ارسال پارامترها برای QdrantClient
            results = strategy.search(
                collection=self.store.client,  # یعنی QdrantClient
                query_vector=query_vector,     # بردار کوئری (embedding)
                k=k,
                min_score=min_score,
                where_filter=where_filter,
                collection_name="rag_embeddings",  # نام کالکشن
                **search_kwargs,
            )
            return results

        except Exception as e:
            LOG.error(f"❌ Error in search jdadıd({search_mode.value}): {e}")
            return []
    ...
    def get_context_for_query(
        ...
        **search_kwargs,
        ...
        results = self.search_similar_chunks_qdrant(
            query,
            search_mode,
            k,
    ...
    def get_context_for_query_qdrant(
        self,
        query: str,
        search_mode: SearchMode = SearchMode.VECTOR,
        k: int = 5,
        ...
        **search_kwargs,
        ...
        """
        Generate a context string for a given query by searching similar chunks in Qdrant.
        """

        try:
        
            if not hasattr(self, "embedding_model"):
                raise ValueError("⚠️ embedding_model در کلاس تعریف نشده است.")
            query_vector = self.embedding_model(query)

            # ✅ فراخوانی متد جدید search_similar_chunks
            results = self.search_similar_chunks_qdrant(
            ...
            # ✅ دریافت چانک‌های اطراف (در صورت نیاز)
            if previous_k > 0 or next_k > 0:
                enhanced_results = self._get_surrounding_chunks(
                    results, previous_k, next_k, active_only, source_filter
                )
            ...
            LOG.error(f"❌ Error in get_context_for_query_qdrant: {e}")
            return {
                "context": "",
                "sources": [],
                "chunk_count": 0,
    ...
    def search_and_generate_answer(
        ...
        **search_kwargs,
        ...
        try:
            ...
            print("44444444")
            context_result = self.get_context_for_query_qdrant(
                query=query,
                search_mode=search_mode,
                k=k,
